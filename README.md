...
| Param | Effect |
|---|---|
| `?seed=demo` | Byte-identical data on every call — safe to snapshot-test against (implies `mode=fast`) |
| `?delay=1500` | Adds artificial latency in ms (max 10s) for exercising loading states |
| `?status=500` | Forces a status code; non-2xx returns a simulated error envelope, 2xx restamps the real body |

```bash
curl "https://your-app.com/api/mock/abc123/1/users?seed=demo&delay=800&status=503"
```
